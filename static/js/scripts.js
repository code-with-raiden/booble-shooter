  ...
  if (!t) {
    ...
    Normal = (facet, autoFlipNormals = false, X1 = 0, Y1 = 0, Z1 = 0) => {
      let ax = 0,
        ...
        az = 0;
      facet.map((q_) => {
        (ax += q_[0]), (ay += q_[1]), (az += q_[2]);
      });
      (ax /= facet.length), (ay /= facet.length), (az /= facet.length);
      let b1 = facet[2][0] - facet[1][0],
        b2 = facet[2][1] - facet[1][1],
        b3 = facet[2][2] - facet[1][2];
      let c1 = facet[1][0] - facet[0][0],
        c2 = facet[1][1] - facet[0][1],
        c3 = facet[1][2] - facet[0][2];
      crs = [b2 * c3 - b3 * c2, b3 * c1 - b1 * c3, b1 * c2 - b2 * c1];
      d = Math.hypot(...crs) + 0.001;
    ...
    reflect = (a, n) => {
      let d1 = Math.hypot(...a) + 0.0001;
      let d2 = Math.hypot(...n) + 0.0001;
      a[0] /= d1;
      a[1] /= d1;
      a[2] /= d1;
      n[0] /= d2;
    ...
    starImgs = Array(9)
      ...
      .map((v, i) => {
        ...
        a.img.onload = () => {
          a.loaded = true;
          setTimeout(() => {
            if (starImgs.filter((v) => v.loaded).length == 9)
              starsLoaded = true;
          }, 0);
        };
    ...
    lineFaceI = (
      ...
      let p2_ = -(
        Math.acos(
          (Y2_ - Y1_) /
            (Math.hypot(X2_ - X1_, Y2_ - Y1_, Z2_ - Z1_) + 0.001)
        ) +
        Math.PI / 2
      );
      let isc = false,
        iscs = [false, false, false];
      (X_ = X1), (Y_ = Y1), (Z_ = Z1);
      R_(0, -p2_, -p1_);
      let rx_ = X_,
        ry_ = Y_,
          ...
          facet.map((q_, j_) => {
            if (isc === false) {
              ...
              let X4_ = X_,
                Y4_ = Y_,
                Z4_ = Z_;
              if ((l_ = I_(X1_, Y1_, X2_, Y2_, X3_, Y3_, X4_, Y4_)))
                iscs[m] = l_;
            }
          });
        }
        ...
        facet.map((q_, j_) => {
          if (pointInPoly) {
            ...
            let X4_ = X_,
              Y4_ = Y_,
              Z4_ = Z_;
            if (I_(X1_, Y1_, X2_, Y2_, X3_, Y3_, X4_, Y4_))
              pointInPoly = false;
          }
        });
        if (pointInPoly) {
        ...
        for (j = rw; j--; )
          for (let i = cl; i--; ) {
            if (parts > 1) {
              ...
              Z = 0;
              R(
                (prl1 = ((Math.PI * 2) / rw) * (j - 1) * twists + t_),
                0,
                0
              );
              tx1 = X;
              ...
              p2a =
                ...
                Math.acos(
                  (ay2 - ay1) /
                    (Math.hypot(ax2 - ax1, ay2 - ay1, az2 - az1) + 0.001)
                );
              ...
              p2b =
                ...
                Math.acos(
                  (ay2b - ay1b) /
                    (Math.hypot(ax2b - ax1b, ay2b - ay1b, az2b - az1b) +
                      0.001)
                );
            }
            a = [];
    ...
    Icosahedron = (size) => {
      ...
      B.map((v) => {
        ...
        a = [
          ...a,
          [cp[idx1a][idx1b], cp[idx2a][idx2b], cp[idx3a][idx3b]],
        ];
      });
      out = [...out, ...a];
      return out;
    ...
    subDividedIcosahedron = (size, subs, sphereize = 0) =>
      subbed(subs, size, sphereize, Icosahedron(size));
    subDividedTetrahedron = (size, subs, sphereize = 0) =>
      subbed(subs, size, sphereize, Tetrahedron(size));
    subDividedOctahedron = (size, subs, sphereize = 0) =>
      subbed(subs, size, sphereize, Octahedron(size));
    subDividedCube = (size, subs, sphereize = 0) =>
      subbed(subs, size, sphereize, Cube(size));
    subDividedDodecahedron = (size, subs, sphereize = 0) =>
      subbed(subs, size, sphereize, Dodecahedron(size));
    ...
    drawSphere = (tx, ty, tz, rad) => {
      if (1 || Math.hypot(tx, ty, tz) < trad2) {
        ...
        if (Z > 0) {
          ...
          forSort = [
            ...forSort,
            [Q(), Z, forSort.length, X1, Y1, Z1, ls],
          ];
        }
      }
    };
    ...
    loadB = () => {
      ...
      B = Array(cl * rw)
        ...
        .map((v, i) => {
          X =
            ((i % cl) - cl / 2 + 0.5) * sp * 1.5 +
            (((i / cl) | 0) % 2 ? (sp * 1.5) / 2 : 0) -
            sp / 2.66;
          Y =
            (((i / cl) | 0) - rw / 2 + 0.5) * sp * (0.75 ** 0.5 / 2) -
            sp * (0.75 ** 0.5 / 2) * ofy;
          Z = 0;
          id =
            Rn() < sFreq
              ? -((1 + Rn() ** 2 * sNum) | 0)
              : (Rn() * sphereSrc.length) | 0;
          vx = vy = vz = 0;
    ...
    advanceRows = () => {
      ...
      for (j = 0; j < cl; j++) {
        let i = j + (rct % 2 ? 0 : cl);
        ofy_ = i % cl != 0 || !(rct % 2) ? 0 : sp * (0.75 ** 0.5 / 2);
        X =
          ((i % cl) - cl / 2 + 0.5) * sp * 1.5 +
          (((i / cl) | 0) % 2 ? (sp * 1.5) / 2 : 0) -
          sp / 2.66;
    ...
    checkCompletions = (X, Y, Z, id) => {
      ...
      if (
        memo.filter((v) => Math.hypot(v[0] - X, v[1] - Y, v[2] - Z) < 0.1)
          .length
      )
        return;
      memo = [...memo, [X, Y, Z]];
      B.map((v, i) => {
        if (id == v[6] || v[6] < 0) {
          ...
          tz = v[2];
          if (
            (d = Math.hypot(tx - X, ty - Y, tz - Z)) < iBr * 1.1 &&
            d > 0.9
          ) {
            if (v[6] < 0) {
                ...
                switch (v[6]) {
                  case -2:
                    ...
                    B.map((q, j) => {
                      if (
                        q[1] > -6 &&
                        Math.hypot(tx - q[0], ty - q[1], tz - q[2]) <
                          iBr * 4
                      ) {
                        cull = [...cull, j];
                      }
    ...
    dropPieces = (idx) => {
      ...
      B.map((v, i) => {
        ...
        if (i != idx) {
          ...
          Y2 = B[idx][1];
          Z2 = B[idx][2];
          if ((d = Math.hypot(X2 - X1, Y2 - Y1, Z2 - Z1)) < iBr * 1.5)
            dropPieces(i);
        }
      });
    };
    ...
    placeCurShot = () => {
      ...
      for (i = cl * (rw + 1) * 3; i--; ) {
        ...
        Z = 0;
        if (
          (d = Math.hypot(X1 - X, Y1 - Y, Z1 - Z)) < mind &&
          !B.filter((v) => {
            return Math.hypot(v[0] - X, v[1] - Y, v[2] - Z) < 0.1;
          }).length
        ) {
          mind = d;
          idx = i;
      ...
      X =
        ...
        sp / 2.66;
      Y =
        ((((i - cl) / cl) | 0) - rw / 2 + 0.5) * sp * (0.75 ** 0.5 / 2) -
        sp * (0.75 ** 0.5 / 2) * ofy +
        ofy2_;
      Z = 0;
      vx = vy = vz = 0;
...
Y4 = Y3 + vy*/
  ...
  if (curShot.fired) {
    ...
    B.map((v) => {
      ...
      if (
        !shotPlaced &&
        (d = Math.hypot(X2 - X1, Y2 - Y1, Z2 - Z1)) < iBr * 0.9
      ) {
        curShot.vx = 0;
        curShot.vy = 0;
        curShot.vz = 0;
  ...
  if (roundover) {
    ...
    if (victory) {
      ...
      x.strokeText(
        "👍",
        c.width / 2,
        c.height / 2 - fs / 2 - fs + fs / 30
      );
      x.fillText("👍", c.width / 2, c.height / 2 - fs / 2 - fs + fs / 30);
      x.strokeText(
        "victory!",
        c.width / 2,
        c.height / 2 - fs / 2 + fs / 30
      );
      x.fillText(
        "victory!",
        c.width / 2,
  ...
  x.fillText(`high-score ${highScore}`, c.width * 0.6, c.height - fs / 3);
  x.textAlign = "center";
